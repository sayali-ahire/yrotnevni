var inventoryApp = angular.module('inventoryApp', ['ngCart', 'ui.bootstrap', 'chart.js', 'ngCookies', 'ngSanitize', 'ngRoute', 'angularUtils.directives.dirPagination', 'angular-loading-bar', 'ngAnimate']);
...
inventoryApp.config(['$routeProvider', '$locationProvider', '$httpProvider', function ($routeProvider, $locationProvider, $httpProvider) {
    ...
    $routeProvider.when('/', {
        ...
        templateUrl: 'template/NewIndex.html',
    }).when('/inventory', {
        title: baseTitle + ' - inventory',
        templateUrl: 'template/inventory.html',
        activetab: 'inventory',
        controller: 'InventoryController'
    }).when('/inventory/edit/:inventory_id', {
        title: baseTitle + ' - Edit Product',
        templateUrl: 'template/inventory-edit.html',
        controller: 'InventoryController',
        activetab: 'inventory'
    });
...
inventoryApp.controller('ctrlApp', ['$scope', '$rootScope', '$location', 'ngCart', '$http', 'invenServ', '$route', 'cfpLoadingBar', '$timeout', 'AuthenticationService', function ($scope, $rootScope, $location, ngCart, $http, invenServ, $route, cfpLoadingBar, $timeout, AuthenticationService) {
    ...
    var urlServer = 'http://localhost/inventoryApp/server/';
    $scope.productform = {};
    $scope.productform.base_selected = 51;
    $scope.msg = false;
        ...
        $http.get(urlServer + 'transaksi.php?action=get')
            .success(function (data, status, headers, config) {
                $timeout(function () {
                    ...
                    $scope.$apply();
                    $scope.rowtrans = $scope.transaksi.length;
                });
...
inventoryApp.controller('InventoryController', ['invenServ', '$scope', '$routeParams', '$http', '$timeout', function (invenServ, $scope, $routeParams, $http, $timeout) {
    ...
    $scope.intial_available_stock = 0;
    $scope.listInventory = function () {
        $http.get(urlServer + 'list.php?action=get_inv').success(function (data) {
            $scope.inventory = data;
        });
    };
    $scope.listInventory();
    $scope.addInventory = function () {
        $http.post(urlServer + 'list.php?action=add_inv', $scope.inventoryform)
            .success(function (response) {
                $timeout(function () {
                    $scope.listInventory();
                    if (response) {
    ...
    $scope.deleteInventory = function (idproduk) {
        if (confirm('do you sure to delete this data?')) {
            $http.get(urlServer + 'list.php?action=deleteInventory&id=' + idproduk)
                .success(function (response) {
                    if (response) {
                        $scope.listInventory();
                    };
    ...
    if (typeof $routeParams.inventory_id != 'undefined') {
        //  $scope.editInventory = function () {
        $http.get(urlServer + 'list.php?action=getdetail_inv&id=' + $routeParams.inventory_id).success(function (data) {
            $scope.invdetail = data;
            console.log($scope.invdetail)
            $scope.intial_available_stock = $scope.invdetail[0].available_stock;
            console.log($scope.intial_available_stock)
        });
        // }
    } 
     $scope.update = function (p) {
         ...
         $scope.close = function () {
            if($scope.msg)
            {
                window.location="http://localhost/inventoryApp/#/inventory";
            }
            $scope.msg = false;
            $scope.product_error = false;
        }
        $scope.calculate_staock = function(p, index){
            
            if(p.stok != '')
            $scope.invdetail[index].available_stock = parseInt($scope.invdetail[index].stok) + parseInt($scope.intial_available_stock);
            //$scope.p.stok = p.stok + p.available_stock
        }
}]);
